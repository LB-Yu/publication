...
def test_exp():
    ...
    executor = ad.Executor([x4, x1_grad])

    x1_val = 2 * np.ones(3)
    x4_val, x1_grad_val = executor.run(feed_dict={x1: x1_val})

    assert isinstance(x4, ad.Node)
    assert np.allclose(x4_val, np.exp(x1_val) * (np.exp(x1_val) + 1))
    assert np.allclose(x1_grad_val, 2 * np.exp(2 * x1_val) + np.exp(x1_val))
