...
class MatMulOp(Op):
    ...
    def __call__(self, node_A, node_B, trans_A=False, trans_B=False, name=None):
...
class ExpOp(Op):
    """op that calculate the exp value"""
    def __call__(self, node_A, name=None):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
        if name is not None:
            ...
            new_node.name = "ExpOp(%s)" % node_A.name
        return new_node
...
sigmoidcrossentropy_op = SigmoidCrossEntropyOp()
